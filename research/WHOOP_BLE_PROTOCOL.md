...
> Reverse-engineered from live Bluetooth HCI snoop logs captured on Android.
> Covers WHOOP 4.0 (hardware codename "Harvard"), Maverick, Goose, and Puffin generations.
> Sufficient for an Android developer to implement a full companion app from scratch.
...
Custom lookup table (identified from HCI snoop pattern analysis):
...
Mapped from HCI snoop traffic. The **cmd byte** is the 3rd field in each observed outgoing frame.
...
> Observed from HCI snoop. Total inner content = 1928 bytes (Gen4).
...
> Observed from HCI snoop. Total inner content = 1932 bytes (Gen4).
...
> Observed from HCI snoop. Total inner content = 1244 bytes.
...
> Observed from HCI snoop. Total inner content = 2140 bytes.
> **Requires:** `TOGGLE_PERSISTENT_R20` (0x99, `[0x01]`).
> No additional field structure observed beyond the common header in captured traffic. Treat as raw optical data buffer.

### 11.5 R7 — Legacy Sensor

> Observed from HCI snoop. Total inner content = 1936 bytes.
...
> Observed from HCI snoop. All offsets below are from `payload[0]` = `frame[7]`.
...
Observed from HCI snoop. The **Int Value** is what appears in `frame[6..7]` (uint16 LE).
